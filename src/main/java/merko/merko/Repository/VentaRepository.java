...
public interface VentaRepository extends JpaRepository<Venta, Long> {
    ...
    long countByFechaBetweenAndEstado(LocalDateTime start, LocalDateTime end, EstadoVenta estado);

    @Query("select function('date', v.fecha) as fecha, coalesce(sum(v.total),0) as total from Venta v where v.estado = :estado and v.fecha between :start and :end group by function('date', v.fecha) order by fecha")
